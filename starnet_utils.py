...
def list_train_images(addr):
    # this function will open only .tif and .tiff images (8 bit per channel)
    original_files = [f for f in listdir(addr + "/original/") if isfile(join(addr + "/original/", f)) and (f.endswith(".tif") or f.endswith(".tiff"))]
    starless_files = [f for f in listdir(addr + "/starless/") if isfile(join(addr + "/starless/", f)) and (f.endswith(".tif") or f.endswith(".tiff"))]
...
def list_test_images(addr):
    # this function will open only .tif and .tiff images (8 bit per channel)
    files = [f for f in listdir(addr) if isfile(join(addr, f)) and (f.endswith(".tif") or f.endswith(".tiff"))]
    return files
